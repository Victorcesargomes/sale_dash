...
# Transformando tudo em int tudo o que der
...
def convert_to_text(month):
    lista1 = ['Ano Todo', 'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto',
...
def graph5(month, toggle):
    ...
    fig5.add_trace(go.Indicator(mode='number+delta',
                            title= {"text": f"<span style='font-size:80%'>{df_5['Consultor'].iloc[0]} - Top Consultor</span><br><span style='font-size:60%'>Em Vendas - em relação à média</span><br>"},
    ...
    fig6.add_trace(go.Indicator(mode='number+delta',
                            title= {"text": f"<span style='font-size:80%'>{df_6['Equipe'].iloc[0]} -Top Equipe</span><br><span style='font-size:60%'>Em vendas - em relação à média</span><br>"},
...
def graph7(toggle):
    ...
    fig7.update_layout(main_config, yaxis={'title': None}, xaxis={'title': None}, height=200, template=template)
    fig7.update_layout({"legend": {"yanchor": "top", "y":0.99, "font": {"color": "white", 'size':12}}})
...
def graph111(month, team, toggle):
    template = template_theme1 if toggle else template_theme2
